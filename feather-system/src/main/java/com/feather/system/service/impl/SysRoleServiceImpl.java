...
public class SysRoleServiceImpl implements ISysRoleService {
    ...
    public int insertRole(SysRole role) {
        ...
        if (role.getStatus() == null) {
            role.setStatus(FeatherConstants.TRUE_SUCCESS_ENABLED_PRIMARY);
        }
        // 新增角色信息
     ...
     */
    @Override
    public List<SysRole> flagAllByIds(Long[] ids) {
        List<SysRole> roles = selectRoleAll();
        for (SysRole role : roles) {
            for (Long i : ids) {
                if (role.getRoleId().longValue() == i.longValue()) {
                    role.setFlag(true);
                    break;
                }
            }
     ...
     */
    @Override
    public List<SysRole> flagAllByKeys(String[] keys) {
        List<SysRole> roles = selectRoleAll();
        for (SysRole role : roles) {
            for (String k : keys) {
                if (role.getRoleKey().equals(k)) {
                    role.setFlag(true);
                    break;
                }
            }
     ...
     */
    @Override
    public List<SysRole> flagAllByUserId(Long userId) {
     ...
     */
    @Override
    public List<SysRole> flagAllByResourceId(Long resourceId) {
        List<SysRole> resRoles = roleMapper.selectRolesByResourceId(resourceId);
        List<SysRole> roles = selectRoleAll();
        for (SysRole role : roles) {
            for (SysRole resRole : resRoles) {
                if (role.getRoleId().longValue() == resRole.getRoleId().longValue()) {
                    role.setFlag(true);
                    break;
                }
            }

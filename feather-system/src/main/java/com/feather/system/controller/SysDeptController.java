...
public class SysDeptController extends BaseController {
    ...
    public AjaxResult addSave(@Validated SysDept dept) {
        ...
        if (!deptService.checkDeptCodeUnique(dept)) {
            throw new RuntimeException("新增部门'" + dept.getDeptName() + "'失败，部门代码已存在");
        }
    ...
    public AjaxResult editSave(@Validated SysDept dept) {
        if (!deptService.checkDeptNameUnique(dept)) {
            throw new RuntimeException("修改部门'" + dept.getDeptName() + "'失败，部门名称已存在");
        } else if (!deptService.checkDeptCodeUnique(dept)) {
            throw new RuntimeException("修改部门'" + dept.getDeptName() + "'失败，部门代码已存在");
        } else if (dept.getParentId().equals(dept.getDeptId())) {
            throw new RuntimeException("修改部门'" + dept.getDeptName() + "'失败，上级部门不能是自己");
        }
    ...
    public AjaxResult top(@PathVariable("deptId") Long deptId) {
        SysDept dept = deptService.selectDeptById(deptId);
        if (dept == null) {
            throw new RuntimeException("未找到部门");
        }
        dept.setParentId(0L);

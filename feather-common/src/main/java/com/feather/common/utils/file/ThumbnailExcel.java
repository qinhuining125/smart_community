...
public class ThumbnailExcel extends ThumbnailBase {
    ...
    public int of() {
        ...
        try {
            String html = getHtml();
            byte[] data = html.getBytes(FeatherConstants.UTF8);
    ...
    private String getHtml() throws Exception {
        try (InputStream inputStream = new FileInputStream(getOriginal())) {
            try (Workbook wb = WorkbookFactory.create(inputStream)) {
                ...
                sb.append(
                        "<meta http-equiv='Content-Type' content='text/html; charset=utf-8' /><table style='border-collapse:collapse;' width='100%'>");
                Row row = null; // 兼容
                Cell cell = null; // 兼容
